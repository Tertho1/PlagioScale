  ...
  return (
    <div className="page-shell">
      <div className="hero">
        ...
        <p style={{ color: "#64748b", fontSize: 14 }}>Roll: {comparisons[0]?.roll || "—"} · Filename: {comparisons[0]?.original_filename || (comparisons[0]?.filename ? comparisons[0].filename.split("_").slice(3).join("_") : "—")}</p>
      </div>
            ...
            <tbody>
              {comparisons.map((c, i) => (
                <tr key={i}>
                  ...
                  <td style={{ color: "#64748b", fontSize: 13 }}>{c.compared_with_original_filename || (c.compared_with_filename ? c.compared_with_filename.split("_").slice(3).join("_") : "—")}</td>
                  <td>
